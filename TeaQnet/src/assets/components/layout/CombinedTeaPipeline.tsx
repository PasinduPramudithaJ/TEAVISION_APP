...
const CombinedTeaPipeline: React.FC = () => {
  ...
  const [cropRaw, setCropRaw] = useState(false);
  ...
  const parseCSVText = async (text: string, previews: string[]): Promise<PredictionRow[]> => {
    ...
    try {
      ...
      for (const f of files) {
        ...
        if (cropRaw) {
          ...
          const base64 = data.cropped_image.split(",")[1];
          const blob = new Blob([Uint8Array.from(atob(base64), c => c.charCodeAt(0))], { type: "image/png" });
          const croppedFile = new File([blob], f.name.replace(/\.\w+$/, ".png"), { type: "image/png" });
      ...
      const parsedRows = await parseCSVText(text, previews);
      setRows(parsedRows);
  ...
  const handlePredictAll = async (rowsToPredict?: PredictionRow[]) => {
    ...
    try {
      ...
      setRows(data.results.map((r: any, i: number) => ({...r, previewUrl: targetRows[i].previewUrl})));
    } catch (err: any) {
      alert(err.message);
    ...
    <>
      ...
      <div className="flex-grow-1 d-flex flex-column align-items-center justify-content-center text-center py-5"
        ...
        <div className="card shadow-lg p-4 mb-4 d-flex flex-column align-items-center flex-wrap"
          ...
          <div className="form-check mt-3 text-start w-100">
            <input
              className="form-check-input"
              type="checkbox"
              id="cropRaw"
              checked={cropRaw}
              onChange={e => setCropRaw(e.target.checked)}
            />
            <label className="form-check-label" htmlFor="cropRaw">
              Crop raw images before feature extraction
            </label>
          </div>

          <div className="d-flex gap-2 mt-3 flex-wrap">
            <button className="btn btn-success rounded-pill d-flex align-items-center gap-2" disabled={!files.length||isExtracting} onClick={handleExtractFeatures}>
              ...
              <div className="table-responsive" style={{ maxHeight:"500px", overflowY:"auto" }}>
                <table className="table table-hover align-middle mb-0">
                  <thead style={{ position:"sticky", top:0, backgroundColor:"white" }}>
                    <tr>
                      <th>#</th><th>Image</th><th>Label</th><th>R</th><th>G</th><th>B</th>
                      <th>H</th><th>S</th><th>V</th><th>Region</th>
                      <th>Group</th><th>Pred Region</th><th>Pred Group</th><th>Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rows.map((r,i)=>{
                      ...
                      return (
                        <tr key={i}>
                          <td>{i+1}</td>
                          <td>
                            {r.previewUrl ? <img src={r.previewUrl} alt={r.path} style={{width:"60px",height:"60px",objectFit:"cover",borderRadius:"4px"}}/> : "-"}
                          </td>
...
export default CombinedTeaPipeline;